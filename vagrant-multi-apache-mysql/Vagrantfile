...
Vagrant.configure("2") do |config|
  ...
  # Configure the Database
  config.vm.define "DBExampleBox2" do |db2|
    db2.vm.provider :virtualbox do |vb2|
      vb2.name = "DBBox2"
      vb2.memory = 1024
      vb2.customize ["modifyvm", :id, "--description", "DBBox is a mysql VM used to demonstrate Infrastructure as code principles via Vagrant. It is also used to demonstrate defining multiple machines in a single Vagrantfile."]
    end

    db2.vm.hostname = "DB2"
    db2.vm.provision :shell, path: "db_provision.sh"
    db2.vm.network "private_network", ip: "192.168.3.7"
   
    # display this message at end of vagrant up
    config.vm.post_up_message = "Successfully started DB2."
  end
